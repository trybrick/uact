      ...
      function actionHandler(e) {
        ...
        if (tagName === 'input') {
          ...
          if (opt) {
            ...
            evt.label = "".concat(evt.action, "_").concat(opt.value);
          }
        } else if (btn) {
        ...
        if (uactOpts.site) {
          ...
          for (var propName in uae) {
            if (uae[propName] === null || uae[propName] === undefined) {
              ...
              uae[propName] = (uae[propName] || '').toLowerCase().replace(/[^0-9a-z_-]+/g, '-').replace(/-{2,}/g, '-');
            }
          }

          uae.pg = document.location.pathname.toLowerCase();
          that.pushEvent("https://pi.brickinc.net/shake/?n=bake&".concat(wu.queryStringify(uae).toLowerCase()));
